...
- Name: tests/student_profile.spec.ts >> Блок 2.1: Профиль студента >> 2.1.1.2: Валидация СНИЛС на фронтенде
- Location: tests/student_profile.spec.ts:22:3
...
Call log:
  - waiting for locator('[data-testid="snils-input"]')
    - waiting for" http://localhost:5173/student" navigation to finish...
    - navigated to "http://localhost:5173/student"
...
```ts
  ...
  2   | 
  3   | test.describe('Блок 2.1: Профиль студента', () => {
  4   |   
  5   |   test.beforeEach(async ({ page }) => {
  6   |     await page.goto('http://localhost:5173/login');
  7   |     await page.locator('input[type="email"]').fill('arhipov_kyu@luberteh.ru');
  8   |     await page.locator('input[type="password"]').fill('student2026');
  9   |     await page.locator('button:has-text("Войти")').click();
